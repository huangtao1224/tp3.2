...
function doing($contact){
    $list=[];
    //给每个数组添加字段 chart 值为所属的A-Z的字母分类
    foreach($contact as &$v){
        $v['chart']=_getFirstCharter($v['input_name']);
    }
    $data=[];
    //给所有数组进行A-Z分类
...
function getInitials($str){
    ...
    $s2  = iconv('GBK', 'UTF-8', $s1);
    $s   = $s2 == $str ? $s1 : $str;
    if(ord($str)>128){//汉字开头，汉字没有以U、V开头的
        $asc=ord($s{0})*256+ord($s{1})-65536;
...
function fckeditor2($name,$height,$value=''){
/*
    include_once 'Admin/Common/fckeditor/fckeditor.php';//引用FCKeditor.php
    $fck = new FCKeditor($name);
    $fck->BasePath= 'Admin/Common/fckeditor/';
...
*/
    //$editorid='editor'.mt_rand(100000,999999);
    echo '<textarea id="content" name="'.$name.'" style="width:100%;height:'.$height.'px;">'.$value.'</textarea>';
}
...
function pinyin($str, $ret_format = 'all', $placeholder = '_', $allow_chars = '/[a-zA-Z\d ]/') {
    ...
    return substr(rtrim($rs, ' '),0,1);
}
function showmsg2($msg,$gourl,$limittime=0,$is=1)
{
    $company = M('site')->getField('company')?:'百家骏网络';
    $htmlhead  = "<html><head><title>".$company."-网站管理系统</title><meta http-equiv='Content-Type' content='text/html; charset=utf-8' /><meta name='viewport' content='width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0'><link href='Admin/Tpl/font-awesome-4.5.0/css/font-awesome.min.css' rel='stylesheet' type='text/css' />";
    $htmlhead .= "<base target='_self'/>\r\n<style>body{background: #f1f1f1;height: 100%;}.box{width: 100%;max-width: 700px;padding:3px;position: relative;top: 50%;left: 50%;transform: translate(-50%,-50%);}</style></head>\r\n<body>\r\n<script>\r\n";
    $htmlfoot  = "</script>\r\n</body>\r\n</html>\r\n";
    if($limittime==0) {
        $litime = 1000;
    }else {
    ...
    if($gourl==''||$onlymsg==1) {
      ...
      function JumpUrl(){
        ...
        if(strstr($gourl,"m=1"))
        {
            $rmsg .= "document.write(\"<div class='box'><div>";
        }
        else
        {
            $rmsg .= "document.write(\"<div class='box'>";
        }
        -
        $rmsg .= "<img src='Admin/Tpl/images/flight.png' width='100%'>\");\r\n";
        $rmsg .= "document.write(\"<div style='margin: 40px 0;'><div style='display: flex;align-items: center;justify-content: center;font-size: 26px;margin-bottom: 30px;color: #9f9f9f;text-shadow: 1px 6px 3px #cdc9c9;'>\");\r\n";
        if($is==1){
            $rmsg .= "document.write(\"".str_replace("\"","","<i class='fa fa-check-circle' style='margin-right: 10px;font-size: 30px;'></i> ".$msg)."\");\r\n";
        }else{
            $rmsg .= "document.write(\"".str_replace("\"","","<i class='fa fa-warning' style='margin-right: 10px;font-size: 30px;'></i>".$msg)."\");\r\n";
        }
        $rmsg .= "document.write(\"";
        if($onlymsg==0)
        {
            if($gourl!='javascript:;' && $gourl!='')
            {
                $rmsg .= "</div><a href='{$gourl}' style='display: block;text-align: center;color: #35a8ff;text-decoration: none;'>如果你的浏览器没反应，请点击这里......</a>";
            }
            $rmsg .= "<br/></div>\");\r\n";
            if($gourl!='javascript:;' && $gourl!='')
            {
