...
<div class="content">
        ...
        <div class="right_data">
            <form method="post" id="form_menu" action="admin.php?m=classify&a=batch_edit_save" class="white">
                <table class="table">
                    ...
                    <?php
 $classify_add = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=2')->find(); $classify_edit = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=3')->find(); $classify_del = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=4')->find(); $classify_con = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=5')->find(); $classify = M('classify')->field('classify_id,level_id,classify_pid,type_id,classify_name,date,is_delete,classify_img,note,page_img')->where(array('classify_id'=>$recursive_classify_id,'level_id'=>array('gt',1)))->order('date asc')->select(); foreach($classify as $k=>$v){ ?>
                    <tr class="row_list" cate-id="<?php echo $v['classify_id'];?>" fid="<?php echo $v['classify_pid'];?>" <?php if($v['level_id']>3){echo 'style="display:none;"';}?>>
                        ...
                        <td class="row_recursive" status="<?php echo $count?'false':'true';?>">
                            ...
                            <?php
 $arr=array('8888'); if(in_array($v['classify_id'],$arr)){ ?>
 ...
 $input_p = M('input')->where(array('input_pid'=>351))->select(); foreach($input_p as $k2=>$v2){ ?>
                        ...
                        <td>
                            ...
                            <?php }?>
                            <?php $classify = M('classify')->where(array('classify_pid'=>$v['classify_id']))->find(); if((empty($classify)&&$count==0&&$v['is_delete']!=2&&$user['permissions']!=3)||($classify_del&&empty($classify)&&$count==0&&$v['is_delete']!=2)){?>
                    ...
                    <?php }?>
                    <?php
 global $list;global $classify_add2;global $classify_edit2;global $classify_del2;global $classify_con2;global $user2; $list = M('classify')->field('classify_id,level_id,classify_pid,type_id,classify_name,date,is_delete,classify_img,note,page_img')->where(array('version_id'=>$version_id))->order('date asc')->select(); $classify_add2 = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=2')->find(); $classify_edit2 = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=3')->find(); $classify_del2 = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=4')->find(); $classify_con2 = M('permission')->where('classify_id='.pg('recursive_classify_id').' and user_id='.$user['user_id'].' and permission_name=5')->find(); $user2 = M('user')->where('user_id='.session('user')['user_id'])->find(); function recursive_classify($classify_pid='1') { global $list;global $classify_add2;global $classify_edit2;global $classify_del2;global $classify_con2;global $user2; foreach($list as $k=>$v) { if($v['classify_pid']==$classify_pid) { ?>
                    <tr class="row_list" style="<?php if($v['classify_id']==$_SESSION['classify_id']){echo 'background: #ececec;';}?>" cate-id="<?php echo $v['classify_id'];?>" fid="<?php echo $v['classify_pid'];?>">
                        ...
                        <td class="class_name cid_<?php echo $v['level_id'];?>" status="false">
                            ...
                            <?php }?>
                            <?php
 $arr=array('8888'); $zi=M('classify')->where(array('classify_pid'=>$v['classify_id']))->find(); if($v['level_id']==3 && $zi){ ?>
                            <?php } ?>
                            <a href="javascript:;"><?php if(!$count){echo "|--";} echo $v['classify_name']?></a>
                            <?php if($v['note']!='')echo '<span class="classify_note">['.$v['note'].']</span>';?>
                            <?php if($v['classify_img']!='')echo '<a href="'.$v['classify_img'].'" target="_blank"><img src="'.$v['classify_img'].'" width="50" height="20"/></a>';?>
                            <?php if($v['page_img']!='')echo '<a href="'.$v['page_img'].'" target="_blank"><img src="'.$v['page_img'].'" width="50" height="20"/></a>';?>
                        </td>
 ...
 $input_p = M('input')->where(array('input_pid'=>351))->select(); foreach($input_p as $k2=>$v2){ ?>
                        ...
                        <td>
                            ...
                            <?php }?>
                            <?php $classify = M('classify')->where(array('classify_pid'=>$v['classify_id']))->find(); if((empty($classify)&&$count==0&&$v['is_delete']!=353&&$user2['permissions']!=3)||(empty($classify)&&$count==0&&$v['is_delete']!=353&&$classify_del2)){?>
